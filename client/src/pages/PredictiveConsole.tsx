      ...
      <AnimatePresence>
        {hoveredPoint !== null && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="absolute top-2 right-2 bg-black/80 border border-purple-500/30 rounded-lg p-3 text-sm"
          >
            ...
            <div className="text-white/50 text-xs mt-1">Key drivers: dev wallet movement, LP imbalance</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
    ...
    <div className="min-h-screen bg-[#050510] text-white">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-black/50 backdrop-blur-md border-b border-white/5">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link href="/" className="text-xl font-light tracking-wide" data-testid="link-home">
            SADS.FUN
          </Link>
      ...
      <main className="relative z-10 pt-24 pb-16 px-6">
        <div className="max-w-4xl mx-auto">
          <header className="text-center mb-12">
            <h1 className="text-4xl md:text-5xl font-light mb-4" data-testid="page-title">
              Predictive Console
            </h1>
          ...
          <div className="mb-12">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <button
                onClick={handleScan}
                disabled={isScanning}
                className="bg-purple-600 hover:bg-purple-500 disabled:bg-purple-600/50 disabled:cursor-not-allowed px-6 py-3 rounded-lg font-medium transition-colors whitespace-nowrap"
          ...
          {!result && !isScanning && !error && (
            <div className="text-center py-16 text-white/40" data-testid="empty-state">
              ...
              <p className="text-sm">Run a scan to see projected risk over time and how this address compares to historical incidents on Solana.</p>
            </div>
          )}
          ...
          {isScanning && (
            <div className="text-center py-16" data-testid="loading-state">
              ...
              <p className="text-white/60">Analyzing on-chain patterns…</p>
            </div>
          )}
          ...
          {result && (
            ...
            >
              <section className="bg-black/30 border border-white/10 rounded-xl p-6" data-testid="prediction-summary">
                <h2 className="text-lg font-medium mb-4">Prediction summary</h2>
                
                <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg border mb-6 ${getRiskBgColor(overallRisk!)}`}>
                  <span className="text-white/60">Overall Risk Forecast:</span>
                  <span className={`font-semibold ${getRiskColor(overallRisk!)}`}>{overallRisk}</span>
                </div>

                <div className="grid md:grid-cols-2 gap-4 mb-4">
                  <div className="bg-black/20 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-white/60 text-sm">Quantum Similarity Score</span>
                      <span className="text-purple-400 font-mono text-lg">{(result.QSS * 100).toFixed(0)}%</span>
                    </div>
                  ...
                  <div className="bg-black/20 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-white/60 text-sm">Temporal Fragility Index (TFI)</span>
                      <span className="text-purple-400 font-mono text-lg">{(result.TFI * 100).toFixed(0)}%</span>
                    </div>
                    <p className="text-xs text-white/40">
              ...
              <section className="grid md:grid-cols-3 gap-4">
                <div className="bg-black/30 border border-white/10 rounded-xl p-5" data-testid="card-temporal-fragility">
                  <h3 className="text-base font-medium mb-3">Temporal Fragility Index</h3>
                  <div className="text-xs text-white/50 mb-2">Stability status</div>
                  <div className={`font-semibold mb-3 ${result.TFI > 0.7 ? 'text-red-400' : result.TFI > 0.5 ? 'text-orange-400' : 'text-yellow-400'}`}>
                    {stabilityStatus}
                  </div>
                  ...
                  <ul className="text-xs text-white/50 space-y-1 mb-4">
                ...
                <div className="bg-black/30 border border-white/10 rounded-xl p-5" data-testid="card-quantum-similarity">
                  <h3 className="text-base font-medium mb-3">Quantum Similarity Score</h3>
                  <div className="text-xs text-white/50 mb-2">Pattern match</div>
                ...
                <div className="bg-black/30 border border-white/10 rounded-xl p-5" data-testid="card-liquidity-holders">
                  <h3 className="text-base font-medium mb-3">Liquidity & Holder Structure</h3>
              ...
              <section className="bg-black/30 border border-white/10 rounded-xl p-6" data-testid="section-risk-chart">
                <h2 className="text-lg font-medium mb-2">Projected risk over time</h2>
                ...
                <div className="flex items-center gap-3 mt-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-white/50">Enable continuous monitoring for this address</span>
                  </div>
                </div>
              ...
              <section className="bg-black/30 border border-white/10 rounded-xl p-6" data-testid="section-historical-cases">
                <h2 className="text-lg font-medium mb-2">Similar historical cases</h2>
                <p className="text-sm text-white/50 mb-4">
                      ...
                      {historicalCases.map((c, i) => (
                        <tr key={i} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                          <td className="py-3 pr-4">
                            <span className="text-purple-400 font-mono">{c.match}%</span>
                          </td>
